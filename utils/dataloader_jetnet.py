...
def custom_collate(data,avg_n,max): #(2)
...
class PointCloudDataloader(pl.LightningDataModule):
   ...
   """
    ...
    def __init__(self,parton,n_dim,n_part,batch_size,sampler, **kwargs):
        super().__init__()
    ...
    def setup(self, stage ,n=None ):
        ...
        self.n = torch.cat((data,test_set),dim=0)[:,:,-1].sum(axis=1)
        self.val_n = test_set[:,:,-1].sum(axis=1)
        self.n_mean = torch.mean(self.n)
        self.avg_n = torch.mean(self.n)
        m=mass(torch.cat((data,test_set),dim=0)[:,:,:3])
        self.n_kde,self.m_kde=fit_kde(self.n,m)
    ...
    def train_dataloader(self):
        if self.sampler:
            return DataLoader(self.data[:,:150],num_workers=16, collate_fn=lambda x:custom_collate(x,self.n_mean,self.max), batch_sampler=self.train_iterator )
        else:
            return DataLoader(self.data[:,:150],num_workers=16, collate_fn=lambda x:custom_collate(x,self.n_mean,self.max),shuffle=True,drop_last=True,batch_size=self.batch_size )

    def val_dataloader(self):
        return DataLoader(self.test_set[:,:150], batch_size=self.batch_size*5, drop_last=False,num_workers=16,collate_fn=lambda x:custom_collate(x,self.n_mean,self.max))

    def test_dataloader(self):
        return DataLoader(self.real_test[:,:150], batch_size=self.batch_size, drop_last=False,num_workers=16,collate_fn=lambda x:custom_collate(x,self.n_mean,False))
