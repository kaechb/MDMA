...
def shower_to_pc(args):
    ...
    return {
        "Egen": torch.tensor(E).squeeze().clone().float(),
        "E_z_alpha_r": pc.clone().float(),
    }
...
class ScalerBase:
    ...
    def save_scalar(self, pcs,save=False):
        ...
        print((orig_pcs-pcs_invert).sum())
        assert (pcs[:,1:].reshape(-1,3).max(0)<[45,16,9]).all() if self.name=="middle" else (pcs[:,1:].reshape(-1,3).max(0)<[45,50,18]).all()
...
if __name__ == "__main__":
    big={"train": ["dataset_3_1.hdf5","dataset_3_2.hdf5"], "test": ["dataset_3_3.hdf5","dataset_3_4.hdf5"]}
    ...
    for middle_dataset in [True]:#,True
        ...
        with torch.no_grad():
            for mode in ["train","test"]:#
                ...
                print("length dataset:",len(outL),len(outD["E_z_alpha_r"]))
