...
class Block(nn.Module):
    def __init__(self, embed_dim, num_heads, hidden,dropout,weightnorm=True,cond_dim=1,glu=False,cloudnorm=False,critic=True):
        super().__init__()
        ...
        if glu:
            self.cond_cls = WeightNormalizedLinear(cond_dim, hidden) if weight_norm else nn.Linear(cond_dim, hidden)
        else:
            self.cond_cls =  WeightNormalizedLinear(cond_dim, hidden) if weight_norm else nn.Linear(cond_dim, hidden)#nn.Linear(cond_dim, hidden)
        self.attn = MultiheadL2Attention(hidden,num_heads) if critic and not weightnorm else weight_norm(nn.MultiheadAttention(hidden, num_heads, batch_first=True, dropout=dropout),"in_proj_weight") if critic else nn.MultiheadAttention(hidden,num_heads,batch_first=True,)
...
class Gen(nn.Module):
    def __init__(self, n_dim, l_dim_gen, hidden_gen, num_layers_gen, heads_gen, cond_dim,cloudnormgen,glu, **kwargs):
        ...
        self.encoder = nn.ModuleList([Block(embed_dim=l_dim_gen, num_heads=heads_gen,hidden=hidden_gen,weightnorm=False,dropout=0,cloudnorm=cloudnormgen,glu=glu,critic=False,cond_dim=cond_dim) for i in range(num_layers_gen)])
...
class Disc(nn.Module):
    def __init__(self, n_dim, l_dim, hidden, num_layers, heads,dropout,cond_dim,weightnorm,cloudnorm,glu, **kwargs):
        ...
        self.encoder = nn.ModuleList([Block(embed_dim=l_dim, num_heads=heads, hidden=hidden,dropout=dropout,weightnorm=weightnorm,cloudnorm=cloudnorm,glu=glu,critic=True,cond_dim=cond_dim) for i in range(num_layers)])
