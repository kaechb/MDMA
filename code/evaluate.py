...
def extract_shower_and_energy(given_file, which):
    ...
    shower = given_file['showers'][:]#[:50000]
    energy = given_file['incident_energies'][:]#[:50000]
...
if __name__ == '__main__':
    ...
    reference_shower, reference_energy = extract_shower_and_energy(reference_file,
                                                                   which='reference')
    shower,energy=shower[:len(reference_shower)][:len(reference_shower)],energy[:len(reference_energy)]
    reference_shower, reference_energy = reference_shower[:len(shower)], reference_energy[:len(energy)]
    ...
    if args.mode in ['all', 'cls-low', 'cls-high', 'cls-low-normed']:
        ...
        classifier= classifier.to(float)
